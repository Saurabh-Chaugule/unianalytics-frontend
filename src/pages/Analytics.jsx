...
import { ArrowLeft, Award, BarChart2 } from 'lucide-react';
...
const Analytics = () => {
  ...
  return (
    <div className="space-y-6 fade-in pb-10">
      <button onClick={() => navigate(-1)} className="flex items-center space-x-2 text-slate-500 hover:text-indigo-500 dark:hover:text-indigo-400 font-bold transition-colors"><ArrowLeft size={18} /> <span>Back</span></button>
      <div><h1 className="text-3xl font-extrabold flex gap-3 text-slate-900 dark:text-white"><Award className="text-indigo-500 drop-shadow-[0_0_10px_rgba(99,102,241,0.5)]" size={32} /> Subject-Wise Marks Analysis</h1></div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
        {subjectStats.length === 0 ? <p className="text-slate-500 dark:text-slate-400 font-bold text-lg">No data available.</p> : subjectStats.map((stat, i) => (
          <div key={i} className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-[0_4px_15px_rgba(0,0,0,0.05)] dark:shadow-[0_4px_20px_rgba(99,102,241,0.15)] hover:-translate-y-2 hover:shadow-[0_8px_30px_rgba(79,70,229,0.15)] dark:hover:shadow-[0_8px_30px_rgba(99,102,241,0.3)] hover:border-indigo-500/50 transition-all duration-300 group">
            <div className="flex justify-between items-start mb-6 border-b border-slate-100 dark:border-slate-700 pb-4"><h3 className="font-extrabold text-lg text-slate-900 dark:text-white group-hover:text-indigo-500 dark:group-hover:text-indigo-400 transition-colors pr-4">{stat.subject}</h3><BarChart2 className="text-indigo-400 dark:text-indigo-500 group-hover:scale-110 transition-transform mt-1 flex-shrink-0" /></div>
